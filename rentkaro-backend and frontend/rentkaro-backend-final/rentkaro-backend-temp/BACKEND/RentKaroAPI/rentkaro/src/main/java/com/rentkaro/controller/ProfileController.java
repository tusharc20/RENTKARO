...
//		return profileService.myProducts(user);
	...
	public List<OrderListDTO> orderHistory(HttpServletRequest request) throws IOException {
//		User user = (User)request.getSession().getAttribute("user");
//		return profileService.myProducts(user);
		Long id = (Long)request.getSession().getAttribute("id");
		List<OrderListDTO> orders = profileService.orderHistory(id);
		for(OrderListDTO order : orders) {
			String path1 = order.getImgPath1();
			order.setProdImg1(readFileToByteArray(new File(path1)));
		}
		return orders;
	}
...
//		return profileService.myProducts(user);
	...
	public List<ProductDTO> rentProduct(HttpServletRequest request) throws IOException {
//		User user = (User)request.getSession().getAttribute("user");
//		return profileService.myProducts(user);
		Long id = (Long)request.getSession().getAttribute("id");
		List<ProductDTO> prods = profileService.rentedProducts(id);
...
//	}
	...
	public void sendMail(Product prod) throws MessagingException {
		...
		String htmlMsg = "Hey " + prod.getOwner().getFirstName()
				...
				+ "<br>Your OTP/Verification code is : <br><h1>"+prod.getOtp().toString().substring(0, 6)
	...
	public String returnProductRequest(HttpServletRequest request) throws MessagingException 
	{
		Long id = (Long)request.getSession().getAttribute("id");
